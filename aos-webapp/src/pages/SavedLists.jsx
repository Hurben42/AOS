  ...
  return (
    <div className="container mt-4 font-monospace pb-5">
      ...
      <div className="row g-3">
        {savedLists.map((list) => (
          <div key={list.id} className="col-12 col-lg-6 text-start">
            <div className="army-card-wrapper position-relative overflow-hidden shadow-lg border border-secondary border-opacity-25 rounded-3">
            ...
            <div className="p-4 pt-2">
              <div className="qr-container-outer mb-4">
                <div className="qr-container-inner bg-white p-3 d-inline-block rounded-4 shadow-lg">
                    {/* On encode les données de la liste dans l'URL pour l'utilisateur qui scanne */}
                    <QRCodeSVG 
                        value={`${window.location.origin}/my-lists/import?data=${encodeURIComponent(JSON.stringify(showQR))}`} 
                        size={160} 
                    />
                </div>
              </div>
      ...
      <style>{`
        ...
        .dock-btn { background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.2); color: #fff; width: 34px; height: 34px; border-radius: 8px; display: flex; align-items: center; justify-content: center; }
